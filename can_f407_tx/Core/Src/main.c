...
uint32_t TxMailbox;
uint8_t TxData[8]; //declare byte to be transmitted //declare a receive byte
...
int main(void)
{
  ...
  pHeader.StdId=0x0; //define a standard identifier, used for message identification by filters (switch this for the other microcontroller)
  ...
  while (1)
  {
    ...
    HAL_CAN_AddTxMessage(&hcan1, &pHeader, TxData, &TxMailbox);
    HAL_Delay(1000);
